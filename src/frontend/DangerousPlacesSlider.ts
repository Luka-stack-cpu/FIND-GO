...
export class DangerousPlacesSlider {
  public static render(
    ...
    const filtered = places.filter(p => 
      dangerousNames.some(name => p.name.toLowerCase().includes(name.toLowerCase()))
    );
    ...
    container.innerHTML = filtered.map(place => {
      ...
      const dots = photos.map((_: any, i: number) => `
        <button class="place-slider-dot${i === 0 ? ' active' : ''}" onclick="event.stopPropagation()"></button>
      `).join('');

      const badgeHtml = SafetyBadge.getHtml(place.safetyScore || 3.4, true);
